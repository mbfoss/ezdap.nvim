...
---@param path? string  defaults to {stdpath("log")}/easydap.log
...
function M.enable(path, min_level)
    ...
    local p = path or (vim.fn.stdpath("log") .. "/easydap.log")
    _file = io.open(p, "a")
    if not _file then
        vim.notify("easydap: cannot open log " .. p, vim.log.levels.ERROR)
        return
    end
    _enabled = true
    write(2, "=== easydap log opened (level=" .. (min_level or "debug") .. ") ===")
end

function M.disable()
    if _enabled then write(2, "=== easydap log closed ===") end

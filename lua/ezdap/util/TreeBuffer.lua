...
---@alias ezdap.util.TreeBuffer.FormatterFn fun(id:any, data:any, expanded:boolean, prefix_width:integer):string[][], string[][], string?
...
---@field indent_guide_hl string?  -- group the guides are drawn with (default EzdapTreeIndentGuide)
...
local _HL_GUIDE = "EzdapTreeIndentGuide"
local _HL_GUIDE_DEFAULT = "EzdapTreeIndentGuideDefault"
...
function TreeBuffer:create_buffer(on_deleted)
    ...
    self._bufnr = uiutil.create_scratch_buffer(false, {
        buftype      = "nofile",
        filetype     = self._filetype or "ezdap-tree",
        modifiable   = false,
        swapfile     = false,
        undolevels   = -1,
    ...
    vim.api.nvim_create_autocmd("BufReadCmd", {
        buffer = self._bufnr,
        callback = function()
            vim.bo[self._bufnr].filetype = self._filetype or "ezdap-tree"
            self:_full_render()
        end,
    })

...
function M.spawn(cmd, opts, bufnr)
    ...
    if type(opts.bufname) == "string"  and opts.bufname ~= "" then
        --- Rename a terminal buffer in place of the opaque autogenerated `term://…`.
        --- Renaming spins the old name off into an unlisted alternate buffer — `#` in
        --- this context, so we delete exactly it. Best-effort: a name clash no-ops.
